# main.py
"""
libclang C Parser API - 安定版
元のAPIに最小限の拡張を加えてループ終了位置検出機能を追加
"""
...
def cursor_to_dict(cursor):
    ...
    # FUNCTION_DECL の returnType を明示的に追加
...
def api_info():
    ...
    return jsonify({
        ...
        "version": "1.1.0",
        "description": "C言語コードの解析結果を取得できるAPI（ループ終了位置検出機能付き）",
        "endpoints": {
            "POST /api/tokens": "字句解析結果（トークン）を取得",
            "POST /api/ast": "構文解析結果（AST）を取得 - 拡張版",
            "POST /api/diagnostics": "診断情報（エラー・警告）を取得", 
            "POST /api/includes": "インクルード情報を取得",
            "POST /api/all": "全ての解析結果をまとめて取得"
        },
        "improvements": {
            "loop_end_detection": "ループの正確な終了位置を検出するためのextent情報を追加",
            "stable_implementation": "最小限の変更で最大限の安定性を確保"
        },
        "request_format": {
        ...
        "example": {
            "url": "/api/ast",
            "method": "POST",
            "body": {"code": "for(int i=0; i<10; i++) { printf(\"%d\", i); }"},
            "note": "extent.end_line にループの正確な終了位置が含まれます"
        }
    })
...
if __name__ == '__main__':
    ...
    print("🚀 libclang C Parser API v1.1 (Stable Enhanced Version)")
    print(f"   Port: {port}")
    print("📍 Available endpoints:")
    print("   GET  / - API information")
    print("   POST /api/tokens - Get lexical analysis results")
    print("   POST /api/ast - Get syntax analysis results (ENHANCED)")  
    ...
    print("\n🎯 Enhancement:")
    print("   ✅ Precise loop end position detection via extent.end_line")
    print("   ✅ Minimal, stable implementation")
